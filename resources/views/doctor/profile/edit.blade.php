...
@section('title', 'Manage Doctor Profile')
    ...
    <div class="bg-gradient-to-r from-sky-500 to-indigo-600 rounded-3xl p-6 md:p-8 mb-8 text-white relative overflow-hidden shadow-xl">
        ...
        <div class="relative z-10 flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-black">Manage Profile</h1>
                    <p class="text-sky-100 text-sm mt-0.5">Keep your details and chambers up to date.</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-4xl mx-auto">
        <form method="POST"
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                ...
                {{-- Photo Upload --}}
                @php
                    $doctorPhotoUrl = $doctor->photo ? asset('storage/' . $doctor->photo) : '';
                @endphp
                <div x-data="{
                ...
                {{-- Cover Image Upload --}}
                @php
                    $doctorCoverUrl = $doctor->cover_image ? asset('storage/' . $doctor->cover_image) : '';
                @endphp
                <div x-data="{
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="name" required value="{{ old('name', $doctor->name) }}"
                    ...
                    <div>
                        <label class="text-xs font-semibold text-gray-600 dark:text-gray-300 block mb-1.5">Phone (Public Profile)</label>
                        <input type="text" name="phone" value="{{ old('phone', $doctor->phone) }}" placeholder="01XXXXXXXXX"
                               class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 focus:bg-white dark:bg-gray-700/50 dark:focus:bg-gray-700 text-gray-700 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-sky-300 transition-colors">
                    </div>
                    <div>
                        <label class="text-xs font-semibold text-gray-600 dark:text-gray-300 block mb-1.5">Email (Public Profile)</label>
                    ...
                    <div class="md:col-span-2">
                        <label class="text-xs font-semibold text-gray-600 dark:text-gray-300 block mb-1.5">Professional Bio</label>
            ...
            @php
                $hospitalsJs = $hospitals->map(function($h) {
                    return [
                        'id' => (string) $h->id, 'name' => $h->name, 'area_id' => (string) $h->area_id,
                        'address' => $h->address, 'phone' => $h->phone, 'google_maps_url' => $h->google_maps_url,
                        'lat' => $h->lat, 'lng' => $h->lng
                    ];
                })->keyBy('id');
                
                $chambersJs = $doctor->chambers->map(fn($c) => [
                        'id'             => $c->id, 'name' => $c->name, 'hospital_id' => (string) $c->hospital_id,
                        'division_id'    => (string) ($c->area?->district?->division_id ?? ''),
                        'district_id'    => (string) ($c->area?->district_id ?? ''),
                        'area_id'        => (string) $c->area_id,
                        'address'        => $c->address, 'visiting_hours' => $c->visiting_hours,
                        'closed_days'    => $c->closed_days ?? '', 'phone' => $c->phone,
                        'google_maps_url'=> $c->google_maps_url ?? '', 'lat' => $c->lat, 'lng' => $c->lng,
                        'districts'      => [], 'areas' => [],
                    ])->toArray();
            @endphp
            <script>
                const setupChamberManager = () => {
                    Alpine.data('chamberManager', () => {
                        return {
                            ...
                            init() { this.chambers.forEach(chamber => this.initChamber(chamber)); },
                            ...
                            addChamber() {
                                this.chambers.push({ id: null, name: '', hospital_id: '', division_id: '', district_id: '', area_id: '', address: '', visiting_hours: '', closed_days: '', phone: '', google_maps_url: '', lat: '', lng: '', districts: [], areas: [] });
                            },
                            removeChamber(i) { this.chambers.splice(i, 1); },
                            async fetchDistricts(index) {
                                let chamber = this.chambers[index];
                                chamber.districts = []; chamber.areas = []; chamber.district_id = ''; chamber.area_id = '';
                ...
                if (window.Alpine) { setupChamberManager(); } else { document.addEventListener('alpine:init', setupChamberManager); }
            </script>

            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6" x-data="chamberManager">
                ...
                <div class="space-y-3">
                    <template x-for="(chamber, index) in chambers" :key="index">
                        <div x-data="{ expanded: true, showAdvanced: false }" class="border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden bg-white dark:bg-gray-800 shadow-sm">
                            <div class="flex items-center justify-between p-4 bg-gray-50/80 dark:bg-gray-700/50 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors" @click="expanded = !expanded">
                                ...
                                <button type="button" @click.stop="removeChamber(index)" class="w-8 h-8 rounded-lg text-gray-400 hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-900/30 flex items-center justify-center transition-colors">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div class="bg-blue-50/50 dark:bg-blue-900/10 p-4 rounded-xl border border-blue-100 dark:border-blue-800/30 md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input type="text" :name="'chambers[' + index + '][name]'" x-model="chamber.name" required placeholder="e.g. Popular Diagnostic Center" class="w-full px-3 py-2 text-sm rounded-lg border border-blue-200 dark:border-blue-700 text-gray-800 dark:text-white bg-white dark:bg-gray-700 focus:ring-2 focus:ring-blue-400">
                                        </div>
                                        <div class="z-20">
                                            <label class="text-xs font-semibold text-gray-600 dark:text-gray-300 block mb-1 flex justify-between">
                                                <span>Link to Hospital / Clinic</span><span class="text-[10px] text-blue-500 font-normal">Auto-fills address & map</span>
                                            </label>
                                            <div x-data="{ open: false, dSearch: '' }" @click.away="open = false" class="relative">
                                                <button type="button" @click="open = !open" class="w-full flex justify-between items-center px-3 py-2 text-sm rounded-lg border border-blue-200 dark:border-blue-700 bg-white dark:bg-gray-700 text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-400 text-left">
                                                ...
                                                <div x-show="open" x-transition.opacity.duration.200ms class="absolute z-50 mt-1 w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl overflow-hidden">
                                                    <div class="p-2 border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-700">
                                                        <input type="text" x-model="dSearch" placeholder="Search hospital..." class="w-full px-3 py-1.5 text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded focus:ring-2 focus:ring-blue-400 focus:outline-none text-gray-800 dark:text-white">
                                                    </div>
                                                    <div class="max-h-48 overflow-y-auto p-1">
                                                        <div @click="chamber.hospital_id = ''; fillHospitalData(index); open = false" class="px-3 py-2 text-sm cursor-pointer rounded hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300">-- Private Chamber --</div>
                                                        <template x-for="h in Object.values(hospitalsData).filter(h => h.name.toLowerCase().includes(dSearch.toLowerCase()))" :key="h.id">
                                                            <div @click="chamber.hospital_id = h.id; fillHospitalData(index); open = false" class="px-3 py-2 text-sm cursor-pointer rounded text-gray-800 dark:text-gray-200" :class="chamber.hospital_id == h.id ? 'bg-blue-50 dark:bg-blue-900/30 font-medium text-blue-600 dark:text-blue-400' : 'hover:bg-blue-50 dark:hover:bg-blue-900/30'"><span x-text="h.name"></span></div>
                                                        </template>
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4 bg-gray-50/50 dark:bg-gray-900/10 p-4 rounded-xl border border-gray-100 dark:border-gray-700">
                                        <div>
                                            ...
                                            <select :name="'chambers[' + index + '][division_id]'" x-model="chamber.division_id" @change="fetchDistricts(index)" class="w-full px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 focus:ring-2 focus:ring-emerald-300">
                                        ...
                                        <div>
                                            ...
                                            <select :name="'chambers[' + index + '][district_id]'" x-model="chamber.district_id" @change="fetchAreas(index)" :disabled="!chamber.division_id" class="w-full px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 focus:ring-2 focus:ring-emerald-300 disabled:opacity-50">
                                        ...
                                        <div>
                                            ...
                                            <select :name="'chambers[' + index + '][area_id]'" x-model="chamber.area_id" :disabled="!chamber.district_id" required class="w-full px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 focus:ring-2 focus:ring-emerald-300 disabled:opacity-50">
                                    ...
                                    <div>
                                        ...
                                        <input type="text" :name="'chambers[' + index + '][phone]'" x-model="chamber.phone" placeholder="01XXXXXXXXX" class="w-full px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/50 focus:bg-white focus:ring-2 focus:ring-emerald-300">
                                    </div>
                                    <div class="md:col-span-2">
                                        <label class="text-xs font-semibold text-gray-600 dark:text-gray-300 block mb-1.5">Full Address</label>
                                        <input type="text" :name="'chambers[' + index + '][address]'" x-model="chamber.address" placeholder="Road 12, House 5, Dhanmondi, Dhaka" class="w-full px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-600 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-emerald-300 dark:bg-gray-700/50">
                                    </div>
                                    <div>
                                        <label class="text-xs font-semibold text-gray-600 dark:text-gray-300 block mb-1.5">📅 Visiting Hours</label>
                                        <input type="text" :name="'chambers[' + index + '][visiting_hours]'" x-model="chamber.visiting_hours" placeholder="e.g. Sat-Thu: 5PM-9PM" class="w-full px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-600 bg-gray-50 focus:bg-white dark:bg-gray-700/50 focus:ring-2 focus:ring-emerald-300">
                                    </div>
                                    <div>
                                        <label class="text-xs font-semibold text-gray-600 dark:text-gray-300 block mb-1.5">🚫 Closed Days</label>
                                        <input type="text" :name="'chambers[' + index + '][closed_days]'" x-model="chamber.closed_days" placeholder="e.g. Friday" class="w-full px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-600 bg-gray-50 focus:bg-white dark:bg-gray-700/50 focus:ring-2 focus:ring-emerald-300">
                                    </div>
                                </div>
                            </div>
                        </div>
                    </template>
                    <div x-show="chambers.length === 0" class="border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-2xl p-8 text-center cursor-pointer hover:border-emerald-300 hover:bg-emerald-50/30 dark:hover:bg-emerald-900/10 transition-colors" @click="addChamber()">
