...
@section('title', 'Edit Ambulance Type')
...
<div class="mb-5 flex items-center gap-3">
    ...
    <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Edit Ambulance Type</h2>
</div>
...
<div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 md:p-8 max-w-5xl">
    <form method="POST" action="{{ route('admin.ambulance-types.update', $ambulance_type->id) }}" class="space-y-6">
        @csrf
        @method('PUT')

        <div>
            <label for="name" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Category Name *</label>
            <input type="text" id="name" name="name" required value="{{ old('name', $ambulance_type->name) }}" placeholder="e.g. AC Ambulance"
        ...
        <div>
            ...
            <textarea id="content" name="content" rows="15" class="w-full">{{ old('content', $ambulance_type->content) }}</textarea>
            @error('content') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
        </div>

        <div class="flex items-center gap-3 mt-4">
            <input type="hidden" name="is_active" value="0">
            <input type="checkbox" id="is_active" name="is_active" value="1" {{ $ambulance_type->is_active ? 'checked' : '' }}
        ...
        <div class="pt-6 border-t border-gray-100 dark:border-gray-700 flex gap-4">
            <button type="submit" class="px-6 py-2.5 bg-gradient-to-r from-sky-500 to-indigo-600 text-white font-semibold rounded-xl text-sm hover:opacity-90 shadow-sm transition-all">Save Changes</button>
