...
<div x-data="{ 
    ...
    <div x-show="showBulkConfirm" style="display: none;" class="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-md">
        <div class="bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-2xl max-w-sm w-full border border-gray-100 dark:border-gray-700 text-center">
            ...
            <form action="{{ route('admin.doctors.bulk-action') }}" method="POST">
                @csrf
    ...
    <table class="w-full text-sm">
        <thead class="bg-gray-50 dark:bg-gray-750 border-b border-gray-100 dark:border-gray-700">
            <tr class="text-left">
                ...
                <th class="px-4 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">#</th>
        ...
        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
            ...
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-750 transition-colors {{ $doctor->featured ? 'bg-amber-50/30 dark:bg-amber-900/10' : '' }}" :class="selectedIds.includes('{{ $doctor->id }}') ? '!bg-sky-50/50 dark:!bg-sky-900/20' : ''">
                <td class="px-4 py-3">
                    <input type="checkbox" value="{{ $doctor->id }}" x-model="selectedIds" class="rounded border-gray-300 text-sky-600 focus:ring-sky-500">
                </td>
                <td class="px-4 py-3 text-xs text-gray-400">{{ $doctors->firstItem() + $loop->index }}</td>
                <td class="px-4 py-3">
                    <div class="flex items-center gap-2.5">
                        <div class="w-8 h-8 rounded-full border border-gray-100 dark:border-gray-700 overflow-hidden flex-shrink-0 bg-gradient-to-br from-sky-100 to-indigo-100 dark:from-sky-900 dark:to-indigo-900 flex items-center justify-center">
                            @if($doctor->photo)
                                <img src="{{ asset('storage/'.$doctor->photo) }}" alt="{{ $doctor->name }}" class="w-full h-full object-cover">
                            @else
                                <span class="text-xs font-bold text-sky-600">{{ mb_substr($doctor->name, 0, 1) }}</span>
                            @endif
                        </div>
                        <div>
                            <p class="font-medium text-gray-700 dark:text-gray-200 text-xs">{{ $doctor->name }}</p>
                            <p class="text-xs text-gray-400 truncate max-w-[160px]">{{ $doctor->designation }}</p>
                        </div>
                    </div>
                </td>
                <td class="px-4 py-3 hidden md:table-cell">
                    <div class="flex flex-wrap gap-1">
                        @forelse($doctor->specialties as $sp)
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800 dark:text-sky-300">{{ $sp->getTranslation('name','en') }}</span>
                        @empty
                ...
                <td class="px-4 py-3">
                    <div class="flex gap-2">
                        <a href="{{ route('doctors.show', $doctor->slug) }}" target="_blank" class="px-2.5 py-1 rounded-lg bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 text-xs hover:bg-emerald-100 transition-colors font-medium" title="View Live">Live</a>
                        <a href="{{ route('admin.doctors.edit', $doctor->id) }}" class="px-2.5 py-1 rounded-lg bg-sky-50 dark:bg-sky-900/30 text-sky-700 dark:text-sky-400 text-xs hover:bg-sky-100 transition-colors font-medium">Edit</a>
                        <button type="button" onclick="confirmDelete('{{ route('admin.doctors.destroy', $doctor->id) }}', 'Delete this doctor?')" class="px-2.5 py-1 rounded-lg bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-400 text-xs hover:bg-red-100 transition-colors font-medium">Delete</button>
                    </div>
                </td>
            </tr>
            @empty
            <tr><td colspan="6" class="px-4 py-12 text-center text-gray-400 text-sm">No doctors found.</td></tr>
            @endforelse
        </tbody>
    </table>
    <div class="px-4 py-3 border-t border-gray-100 dark:border-gray-700">{{ $doctors->links() }}</div>
</div>

    {{-- CSV Import Modal --}}
    <div x-show="openImportModal" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm"
        ...
        <div @click.away="openImportModal = false" class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md p-6 transform transition-all"
             x-transition:enter="transition ease-out duration-200"
