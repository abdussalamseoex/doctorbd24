...
class AIGeneratorController extends Controller
{
    ...
    private function buildPrompt($type, $context)
    {
        ...
        if (in_array($type, ['seo_landing_page', 'seo_title', 'seo_desc', 'seo_faq_schema'])) {
            ...
            $wordCountRule = "\n\nSTRICT WORD COUNT, HEADINGS & BOLD KEYWORD STANDARD FOR PROGRAMMATIC SEO PAGES:\n" .
                "1. If generating TOP CONTENT (above directory list): You MUST write AT LEAST 400 to 450 words. Include structured H2 and H3 subheadings, bolded focus keywords using <strong>...</strong> tag, symptoms/reasons to consult, local healthcare context, and 2-3 internal links.\n" .
                "2. If generating BOTTOM CONTENT (below directory list): You MUST write AT LEAST 1,000 to 1,200 words. Include structured H2 and H3 subheadings, bolded focus keywords using <strong>...</strong> tag, comprehensive patient guides, diagnostic insights, AND AT LEAST 6 COMPREHENSIVE FAQs formatted with clear H4 headings and rich answers.\n" .
                "3. KEYWORD BOLDING: Bold the primary target keywords (e.g. <strong>Cardiologists in Dhaka</strong>, <strong>best doctor chamber</strong>) at least 3-4 times per section.";

            if ($targetLang === 'bengali' || $targetLang === 'bn' || $targetLang === 'bangla') {
                $baseText .= $wordCountRule . "\n\nCRITICAL BANGLADESHI BENGALI WRITING RULE: Write in 100% authentic, natural Bangladeshi Bengali journalism and medical writing style. STRICTLY PROHIBITED: Do NOT use robotic or literal English translations. Keep familiar medical terms in natural Bengali script as Bangladeshi healthcare seekers expect (যেমন: কার্ডিওলজিস্ট, চেম্বার, ইসিজি, ইকোকার্ডিওগ্রাম, হাসপাতাল, ডায়াগনস্টিক সেন্টার).\n\nSTRICT OFFICIAL SITEMAP & FILTER INTERNAL LINKING RULE: When adding internal links, use exact verified official DoctorBD24 sitemap and specialty filter URLs such as: <a href=\"/doctors?specialty=cardiology\" class=\"text-indigo-600 hover:underline font-medium\">কার্ডিওলজি ডাক্তার তালিকা</a>, <a href=\"/hospitals-in-dhaka\" class=\"text-indigo-600 hover:underline font-medium\">হাসপাতাল ও ডায়াগনস্টিক সেন্টার</a>, <a href=\"/specialties\" class=\"text-indigo-600 hover:underline font-medium\">সকল চিকিৎসাসেবা বিভাগ</a>, <a href=\"/ambulances\" class=\"text-indigo-600 hover:underline font-medium\">অ্যাম্বুলেন্স সার্ভিস</a>, or <a href=\"/contact\" class=\"text-indigo-600 hover:underline font-medium\">যোগাযোগ ও সহায়তা</a>. DO NOT invent fake URLs.";
            } else {
                $baseText .= $wordCountRule . "\n\nCRITICAL ENGLISH WRITING RULE: Write in clear, authoritative medical healthcare English tailored for patients in Bangladesh.\n\nSTRICT OFFICIAL SITEMAP & FILTER INTERNAL LINKING RULE: When adding internal links, use exact verified official DoctorBD24 sitemap and specialty filter URLs such as: <a href=\"/doctors?specialty=cardiology\" class=\"text-indigo-600 hover:underline font-medium\">Cardiology Doctor Filter</a>, <a href=\"/hospitals-in-dhaka\" class=\"text-indigo-600 hover:underline font-medium\">Hospitals & Diagnostics</a>, <a href=\"/specialties\" class=\"text-indigo-600 hover:underline font-medium\">Medical Specialties</a>, <a href=\"/ambulances\" class=\"text-indigo-600 hover:underline font-medium\">Ambulance Services</a>, or <a href=\"/contact\" class=\"text-indigo-600 hover:underline font-medium\">Contact Us</a>. DO NOT invent fake URLs.";
            }
        }
