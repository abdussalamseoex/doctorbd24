            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('doctors.show', $hospital->slug) }}" class="ml-1 md:ml-2 hover:text-emerald-600 transition-colors">{{ $hospital->name }}</a>
                    </div>
                </li>
                <li>
                    <div class="flex items-center">
                ...
                <div class="sticky top-24 space-y-5">
                    ...
                    @if(is_null($target->user_id))
                        <div class="bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20 rounded-3xl border border-emerald-100 dark:border-emerald-800/50 p-6 shadow-sm text-center relative overflow-hidden mb-5">
                            ...
                            @auth
                                @php
                                    $hasPendingClaim = auth()->user()->claimRequests()->where('doctor_id', $target->id)->where('status', 'pending')->exists();
                                @endphp
                                @if($hasPendingClaim)
                                            ...
                                            <div x-show="claimModal" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm" x-transition.opacity>
                                                <div @click.away="claimModal = false" class="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden relative" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100">
                                                    ...
                                                    <div class="p-8 text-left">
                                                        ...
                                                        <h3 class="text-2xl font-black text-gray-900 dark:text-white mb-2">{{ __('Doctor Claim Request') }}</h3>
                                                        <p class="text-sm text-gray-500 dark:text-gray-400 mb-6 leading-relaxed">
                                                            {{ __('Please provide a secure message detailing your professional identity. Include official ID reference or authorization letter.') }}
                                                        </p>
                                                        
                                                        <form method="POST" action="{{ route('doctors.claim', $target->id) }}">
                                                            @csrf
                                                            <div class="mb-5">
                                                                ...
                                                                <textarea name="message" rows="4" required placeholder="E.g., I am the doctor of this profile..." class="w-full border border-gray-200 dark:border-gray-700 rounded-2xl px-4 py-3 text-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-gray-100 transition-colors placeholder-gray-400"></textarea>
                                                            </div>
